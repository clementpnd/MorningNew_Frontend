...
function Home() {
  ...
  useEffect(() => {
    fetch(
      "https://morning-new-frontend-84947faap-clementpnd.vercel.app/articlesDb"
    )
  ...
  const displayedlayArticles = articlesData.filter(
    (data) => !hiddenArticles.includes(data.title)
  );

  const articles = displayedlayArticles.map((data, i) => {
